...
| **Методы анализа и отладки БП** | [bp-analysis-methods.md](bp-analysis-methods.md) |
...
1. `.bpt` = **zlib + PHP serialize**, не ZIP → `analyze-bpt-export.js`.

2. Разбор: шаблон + **журнал БП** + **поля документа**.

3. Всегда проверять **`Activated`** на блоке и родителях.

4. SetField **внутри одного** экземпляра ≠ автозапуск на каждое поле.

5. **Не запоминать** конкретные шаблоны в skill — только методы ([bp-analysis-methods.md](bp-analysis-methods.md)).
