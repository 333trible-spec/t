...
(function () {
  ...
  function ownershipHintText(type) {
    if (type === OWNERSHIP.INDIVIDUAL) return '';
  ...
  function renderField(field) {
    ...
    switch (field.type) {
      ...
      case 'file': {
        ...
        const fileInner =
          '<input class="file-native" id="' + fid + '" type="file" name="' + esc(field.code) + '"' + multi + reqAttr + '>' +
          '<label class="file-trigger" for="' + fid + '" title="Прикрепить файл" aria-label="Прикрепить файл">' +
        ...
        if (isDocTile) {
          control =
            ...
            '<div class="file-field" data-file-field="' + esc(field.code) + '">' + fileInner + '</div>' +
            '</div>';
        } else {
          control =
            '<div class="file-field" data-file-field="' + esc(field.code) + '">' + fileInner + '</div>';
  ...
  function renderForm(container, config) {
    ...
    SECTIONS.forEach((sec) => {
      html +=
        ...
        '<h3>' + esc(sec.title) + '</h3>' +
        '<div class="section-progress" data-section-progress="' + esc(sec.id) + '">' +
        '<span class="section-progress-text" data-section-progress-text>0/0</span>' +
        '<div class="section-progress-bar" aria-hidden="true">' +
        '<span class="section-progress-fill" data-section-progress-fill" style="width:0%"></span>' +
        '</div></div></div>' +
  ...
  function refreshProgress(form) {
    ...
    SECTIONS.forEach((sec) => {
      const sectionEl = form.querySelector('[data-section-id="' + sec.id + '"]');
      if (!sectionEl) return;
      let secTotal = 0;
      let secFilled = 0;
      ...
      sec.codes.forEach((code) => {
        if (code === 'ownership_contact') {
          secTotal += 1;
          if (isOwnershipFilled(form)) secFilled += 1;
          return;
        }
        const field = byCode[code];
        if (!field || !isFieldVisible(form, field)) return;
        secTotal += 1;
        if (isFieldFilled(form, field)) secFilled += 1;
      });

      total += secTotal;
      filled += secFilled;

      const progress = sectionEl.querySelector('[data-section-progress="' + sec.id + '"]');
      if (!progress) return;
      const text = progress.querySelector('[data-section-progress-text]');
      const fill = progress.querySelector('[data-section-progress-fill]');
      const pct = secTotal ? Math.round((secFilled / secTotal) * 100) : 0;
      if (text) text.textContent = secFilled + '/' + secTotal;
      if (fill) fill.style.width = pct + '%';
      progress.classList.toggle('section-progress-done', secTotal > 0 && secFilled === secTotal);
    });

    const summary = document.getElementById('form-progress-summary');
    if (summary) {
      summary.textContent = 'Заполнено ' + filled + ' из ' + total;
    }
  }
  ...
  function bindForm(container, config) {
    ...
    form.querySelectorAll('input[type=file]').forEach((inp) => {
      inp.addEventListener('change', () => {
        ...
        if (hint) hint.textContent = names;
        const wrap = inp.closest('.file-field');
        if (wrap) wrap.classList.toggle('has-file', inp.files.length > 0);
        refreshFilledState(form);
      });
    });
