...
(function () {
  ...
  function contactIconSvg() {
    return (
      ...
      '<path d="M5.5 19.5c.3-3.2 2.8-5 6.5-5s6.2 1.8 6.5 5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>' +
      '</svg>'
    );
  }

  function renderContactRequisitesStatus(c) {
    if (c.requisitesFilled === undefined || c.requisitesFilled === null) return '';
    if (c.requisitesFilled) {
      return '<span class="contact-requisites contact-requisites-ok">реквизиты заполнены</span>';
    }
    return '<span class="contact-requisites contact-requisites-miss">реквизиты не заполнены</span>';
  }
  ...
  function renderOwnershipCards(contacts) {
    ...
    return contacts.map((c) => {
      ...
      '<span class="contact-avatar" aria-hidden="true">' + contactIconSvg() + '</span>' +
      '<div class="contact-body">' +
      '<span class="contact-name">' + esc(title) + '</span>' +
      renderContactRequisitesStatus(c) +
      '</div>' +
      '</div>'
      );
    }).join('');
  ...
  function refreshProgress(form) {
    ...
    SECTIONS.forEach((sec) => {
      sec.codes.forEach((code) => {
        if (code === 'ownership_contact') {
          total += 1;
          if (isOwnershipFilled(form)) filled += 1;
          return;
        }
        const field = byCode[code];
        if (!field || !isFieldVisible(form, field)) return;
        total += 1;
        if (isFieldFilled(form, field)) filled += 1;
      });
    });
