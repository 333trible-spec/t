...
(function () {
  ...
  async function fetchContactTitles(call, ids) {
    ...
    for (let i = 0; i < ids.length; i++) {
      try {
        ...
        out.push({ id: Number(c.ID), title: title || 'Контакт #' + c.ID });
      } catch (_) {
        out.push({ id: ids[i], title: 'Контакт #' + ids[i] });
      }
    }
    return out;
  }
  ...
  function setFileHint(form, param, names) {
    ...
    hint.textContent = 'В сделке: ' + names.join(', ') + ' (файл в форму не копируется)';
    hint.classList.add('ok');
  }

  async function buildPayload(deal, enumMaps, call) {
    ...
    for (const row of (window.BP608_DEAL_PREFILL_MAP || []).filter((r) => r.kind === 'crm')) {
      if (!payload[row.param]) continue;
      const contacts = await fetchContactTitles(call, payload[row.param]);
      payload[row.param] = contacts.map((c) => c.id);
      window.__bp608CrmPrefill = window.__bp608CrmPrefill || {};
      window.__bp608CrmPrefill[row.param] = contacts;
    }
  ...
  async function buildDealContactStash(deal, call) {
    const dealContact = deal.CONTACT_ID
      ? (await fetchContactTitles(call, [deal.CONTACT_ID]))[0] || null
      : null;
    const parameter1 = await fetchContactTitles(call, parseCrmContactIds(deal.UF_CRM_1722231741));
    const parameter2 = await fetchContactTitles(call, parseCrmContactIds(deal.UF_CRM_1722231954));
    return { dealContact, parameter1, parameter2 };
  }

  function needsContactTitle(contact) {
    if (!contact || !contact.id) return false;
    const title = String(contact.title || '').trim();
    return !title || /^Контакт #\d+$/i.test(title);
  }

  async function hydrateContactList(call, contacts) {
    const list = (contacts || []).filter((c) => c && c.id > 0);
    if (!list.length) return [];
    if (!list.some(needsContactTitle)) return list;
    return fetchContactTitles(call, list.map((c) => Number(c.id)));
  }
  ...
  window.BP608DealPrefill = {
    ...
    load: async function (form, dealId, call) {
      ...
      const { payload, report } = await buildPayload(deal, enumMaps, call);
      window.BP608Form.applyValues(form, payload);
      await applyCrmPrefill(form);
      await window.BP608DealPrefill.hydrateContacts(form, call);
