...
(function () {
  ...
  function refreshFilledState(form) {
    ...
    if (own) {
      const type = propertyTypeSelected(form)[0];
      let has = false;
      if (type === OWNERSHIP.JOINT) has = crmContactIds(getCrmField(form, 'Parameter1')).length > 0;
      else if (type === OWNERSHIP.SHARED) has = crmContactIds(getCrmField(form, 'Parameter2')).length > 0;
      else if (type === OWNERSHIP.INDIVIDUAL) has = own.querySelectorAll('.contact-card').length > 0;
      own.classList.toggle('filled', has);
    }
  }
