...
(function () {
  ...
  async function loadUserFieldRows(call) {
    ...
    const rows = await call('crm.deal.userfield.list', {});
    window.__bp608UserFields = rows || [];
    return window.__bp608UserFields;
  }
  ...
  async function buildDealUpdateFields(form, formValues, call) {
    ...
    const fields = {};
    const report = { saved: [], files: [], skipped: [], empty: [] };
    ...
    for (const row of window.BP608_DEAL_PREFILL_MAP || []) {
      ...
      if (userType === 'enumeration' || field.type === 'select') {
        if (field.multiple || row.multi) {
          const arr = Array.isArray(val) ? val : [val];
          const ids = arr.map((v) => lookupEnumId(row.deal, v, reverseMaps)).filter(Boolean);
          fields[row.deal] = ids.length ? ids : [];
        } else {
          const single = Array.isArray(val) ? val[0] : val;
          fields[row.deal] = lookupEnumId(row.deal, single, reverseMaps) || String(single).trim();
        }
        report.saved.push(row.param);
        continue;
      }
  ...
  async function isDealCompletedStage(call, deal) {
    if (isWonStageId(deal.STAGE_ID)) return true;
    try {
      if ((await stageSemantics(call, deal)) === 'S') return true;
    } catch (_) {}
    const name = await resolveStageName(call, deal);
    return isCompletedStageName(name);
  }
