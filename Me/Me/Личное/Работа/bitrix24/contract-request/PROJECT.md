...
| | |
|---|---|
| **Папка в vault** | `Me/Me/Личное/Работа/bitrix24/contract-request/` |
...
| Было | Стало |
|------|-------|
| БП 608 запускается штатно (робот / CRM) | Вкладка: только **статус** и **задания** по шаблону 608 |
...
Карточка сделки (ik-navigator)
    └── вкладка «Заявка на договор»  [placement: CRM_DEAL_DETAIL_TAB]
            └── iframe → tab.html (локальное приложение)
                    ...
                    ├── bizproc.workflow.instances — активные процессы (шаблон 608)
                    └── bizproc.task.list / complete — задания по сделке
```

Запуск БП **из вкладки не делаем** — только просмотр и выполнение заданий.
