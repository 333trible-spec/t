// ==UserScript==
// @name         B24 — фон карточки сделки
// @namespace    b24-deal-card-bg
// @version      4.0.0
// @description  Красит фон слайдера CRM по полю UF_CRM_DEAL_CARD_BG (hex)
// @match        https://*.bitrix24.ru/*
// @match        https://*.bitrix24.com/*
// @grant        none
// @run-at       document-idle
// ==/UserScript==
// Tampermonkey → «Создать скрипт» → вставить этот файл целиком → Сохранить

(function () {
  'use strict';
  const PAINTER_VERSION = '4.0.0-us';
    ...
    forEachWindow((win) => {
      ...
      try {
        ...
        for (const slider of list) {
          for (const u of [slider?.getUrl?.(), slider?.getUri?.(), slider?.getFrameWindow?.()?.location?.href]) {
  ...
  function isCrmSliderOpen() {
    ...
    forEachWindow((win) => {
      ...
      if (doc?.querySelector('.ui-side-panel-wrap, .ui-page-slider-wrapper, .crm-entity-widget-content')) open = true;
    });
    return open;
  }
  ...
  function shouldPaintDoc(doc, win) {
    ...
    if (/crm/i.test(href)) return true;
    if (doc.querySelector('.crm-entity-widget-content, .ui-entity-editor, .ui-side-panel-wrap, .ui-page-slider-wrapper')) return true;
  ...
  function paintChainFromEditor(hex, doc) {
    ...
    for (let i = 0; i < 20 && el; i += 1) {
      ...
      if (cls.includes('ui-side-panel') || cls.includes('ui-page-slider') || cls.includes('crm-entity-card')) {
  ...
  async function refresh() {
    ...
    if (!dealId) {
      if (lastHex && isCrmSliderOpen()) paintEverywhere(lastHex, true);
      return;
    }
    activeDealId = dealId;
  ...
  function bindHooks() {
    forEachWindow((win) => {
      try {
        if (!win.BX?.addCustomEvent || win.__b24DealCardBgHooks) return;
