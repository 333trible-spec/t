...
(function () {
  ...
  function contactIconSvg(contactId, incomplete) {
    const gid = 'ndCg' + String(contactId || '0') + (incomplete ? 'a' : '');
    const fill = incomplete
      ? '#f4863f'
      : 'url(#' + gid + ')';
    const defs = incomplete
      ? ''
      : (
    ...
    return (
      '<svg class="contact-icon' + (incomplete ? ' contact-icon--alert' : '') + '" viewBox="0 0 24 24" fill="none" aria-hidden="true">' +
      defs +
      '<circle cx="12" cy="8.4" r="3.9" fill="' + fill + '"/>' +
      '<path d="M4.6 20c.5-3.8 3.2-6 7.4-6s6.9 2.2 7.4 6" fill="' + fill + '"/>' +
      '</svg>'
    );
  }
  ...
  function contactAlertMessages(c) {
    const parts = [];
    if (c.fioFilled !== true) parts.push('Не заполнено ФИО');
    if (c.phoneFilled !== true) parts.push('Не заполнен телефон');
    if (c.emailFilled !== true) parts.push('Не заполнена почта');
    if (c.requisitesFilled !== true) parts.push('Не заполнены реквизиты');
    return parts;
  }

  function contactHasAlerts(c) {
    return contactAlertMessages(c).length > 0;
  }

  function renderContactAlertTip(c) {
    const parts = contactAlertMessages(c);
    if (!parts.length) return '';
    const tip = parts.map((p) => '<span class="contact-alert-line">' + esc(p) + '</span>').join('');
    return '<span class="contact-alert-tip" role="tooltip">' + tip + '</span>';
  }
  ...
  function renderOwnershipCards(contacts, options) {
    ...
    const showLockedRemove = !!opts.showLockedRemove;
    ...
    return contacts.map((c) => {
      ...
      const incomplete = contactHasAlerts(c);
      const alertParts = contactAlertMessages(c);
      const removeIcon =
      ...
      if (canRemove) {
        removeBtn =
          ...
          ' title="Убрать из участников" aria-label="Убрать ' + esc(title) + '">' + removeIcon + '</button>';
      } else if (showLockedRemove) {
        removeBtn =
          '<button type="button" class="contact-remove contact-remove--locked" aria-disabled="true"' +
          ' title="Контакт обязателен на этой стадии сделки"' +
          ' aria-label="Контакт обязателен на этой стадии сделки">' + removeIcon + '</button>';
      }
      const wrapClass = 'contact-card-wrap' + (removeBtn ? ' contact-card-wrap--removable' : '');
      const avatarAria = incomplete
        ? ' tabindex="0" aria-label="' + esc(alertParts.join('. ')) + '"'
        : ' aria-hidden="true"';
      return (
      '<div class="' + wrapClass + '">' +
      ...
      '<span class="contact-avatar' + (incomplete ? ' contact-avatar--alert' : '') + '"' + avatarAria + '>' +
      contactIconSvg(c.id, incomplete) +
      renderContactAlertTip(c) +
      '</span>' +
  ...
  function renderOwnershipDisplay(form, contacts) {
    ...
    if (!contacts || !contacts.length) {
      ...
      box.innerHTML = renderOwnershipCards(contacts, {
        ...
        showLockedRemove: !ownershipAllowsPick(type) && ownershipEditable(form),
      });
    }
    if (hint) {
  ...
  function syncOwnershipContacts(form, options) {
    ...
    contacts = mergeContactFlags(contacts);
  ...
  function normalizeCrmContact(c) {
    if (!c) return null;
    ...
    if (c.lastName != null) out.lastName = c.lastName;
    if (c.firstName != null) out.firstName = c.firstName;
    if (c.secondName != null) out.secondName = c.secondName;
    if (c.fioFilled != null) out.fioFilled = !!c.fioFilled;
    if (c.phoneFilled != null) out.phoneFilled = !!c.phoneFilled;
    if (c.emailFilled != null) out.emailFilled = !!c.emailFilled;
    if (c.requisitesFilled != null) out.requisitesFilled = !!c.requisitesFilled;
    return out;
  }
  ...
  function setCrmField(form, fieldCode, contacts) {
    ...
    const normalized = (contacts || []).map(normalizeCrmContact).filter(Boolean);
  ...
  function mergeContactFlags(contacts) {
    const stash = getDealContactStash();
    ...
    return (contacts || []).map((c) => {
      ...
      return Object.assign({}, s, c, {
        ...
        phoneFilled: c.phoneFilled != null ? c.phoneFilled : s.phoneFilled,
  ...
  function getSendContacts(form) {
    ...
    if (type === OWNERSHIP.JOINT) return mergeContactFlags(getCrmField(form, 'Parameter1'));
    if (type === OWNERSHIP.SHARED) return mergeContactFlags(getCrmField(form, 'Parameter2'));
    return [];
  }

  function contactReadyForSend(c) {
    if (!c || !c.id) return false;
    return c.fioFilled === true
      && c.phoneFilled === true
      && c.emailFilled === true
      && c.requisitesFilled === true;
  }

  function collectContactValidationMessages(form) {
    const contacts = getSendContacts(form);
    const messages = [];
    if (!isOwnershipFilled(form) || !contacts.length) {
      messages.push('Не выбран контакт');
      return messages;
    }
    contacts.forEach((c) => {
      const name = String(c.title || '').trim() || ('Контакт #' + c.id);
      const parts = contactAlertMessages(c);
      if (!parts.length) return;
      if (contacts.length === 1) {
        parts.forEach((p) => messages.push(p));
      } else {
        parts.forEach((p) => messages.push(name + ': ' + p.toLowerCase()));
      }
    });
    return messages;
  }

  function fieldMissingMessage(field) {
    const label = String(field.label || field.code || 'Поле').trim();
    if (field.type === 'file') return 'Не прикреплён файл: «' + label + '»';
    if (field.type === 'select' || isYesNoField(field.code) || field.code === 'property_type') {
      return 'Не выбран: «' + label + '»';
    }
    return 'Не заполнено: «' + label + '»';
  }
  ...
  function refreshSendButton(form) {
    ...
    if (!sendBtn) return;
    if (window.__bp608ViewOnly || form.classList.contains('form-readonly')) {
  ...
  function bindForm(container, config) {
    ...
    if (ownershipField) {
      ownershipField.addEventListener('click', (e) => {
        ...
        const lockedBtn = e.target.closest('.contact-remove--locked');
        if (lockedBtn) {
          e.preventDefault();
          e.stopPropagation();
          return;
    ...
    actionsRoot.querySelector('#btn-validate').onclick = () => {
      if (window.__bp608ViewOnly) return;
      const bad = [];
      config.forEach((field) => {
        const wrap = form.querySelector('[data-field="' + field.code + '"]');
        ...
        if (field.code === 'link_deal' && isRepidSaleYes(form)) {
          ...
          if (!ok) bad.push(fieldMissingMessage(field));
          return;
        }
      ...
      const contactMsgs = collectContactValidationMessages(form);
      const ownWrap = form.querySelector('.ownership-contact-field');
      if (ownWrap) ownWrap.classList.toggle('invalid', contactMsgs.length > 0);
      bad.push.apply(bad, contactMsgs);

      if (bad.length) {
        msg.className = 'form-msg';
        msg.textContent = '';
        if (window.__bp608ShowSaveOverlay) {
          window.__bp608ShowSaveOverlay(bad.join('\n'), 'error');
          setTimeout(() => {
            if (window.__bp608HideSaveOverlay) window.__bp608HideSaveOverlay();
          }, Math.min(8000, 2800 + bad.length * 600));
        }
      } else {
        msg.className = 'form-msg';
        msg.textContent = '';
        ...
        return (async () => {
          ...
          if (!canSendForm(form)) {
            ...
            if (window.__bp608ShowSaveOverlay) {
              ...
              setTimeout(() => {
                if (window.__bp608HideSaveOverlay) window.__bp608HideSaveOverlay();
              }, Math.min(8000, 2800 + contactMsgs.length * 600));
            }
            return;
          }
          ...
          if (window.__bp608ShowSaveOverlay) {
            window.__bp608ShowSaveOverlay('Передаю в оформление. Не закрывайте окно', 'progress');
          }
          try {
            ...
            if (submitBtn) submitBtn.disabled = !!window.__bp608ViewOnly;
            refreshSendButton(form);
          }
        })();
      };
    ...
    function finishSubmit() {
      return (async () => {
        ...
        try {
          ...
          if (submitBtn) submitBtn.disabled = !!window.__bp608ViewOnly;
          if (yesBtn) yesBtn.disabled = !!window.__bp608ViewOnly;
        }
      })();
    }
  ...
  function setReadOnly(container, readOnly, options) {
    ...
    if (bar) {
      ...
      bar.querySelectorAll('.btn-action-edit').forEach((btn) => {
        btn.disabled = !!(viewOnly || readOnly);
      });
    }
  }
